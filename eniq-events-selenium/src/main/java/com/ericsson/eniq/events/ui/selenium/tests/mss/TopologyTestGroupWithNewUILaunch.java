/**
 * -----------------------------------------------------------------------
 *     Copyright (C) 2012 EGI Limited.  All rights reserved.
 * -----------------------------------------------------------------------
 */
 ...
 *
 */
public class TopologyTestGroupWithNewUILaunch extends EniqEventsUIBaseSeleniumTest {
    /*@Autowired
	...
	private String closeButtonXPath =  "//div[contains(@class, 'x-nodrag x-tool-close x-tool')]";
   ...
   // @After
    ...
    public void Topology2digitsofMNCandMCCextractedfromIMSIforMSCeventCallForw_2_8()
        ...
        subscriberEventAnalysis.checkInOptionalHeaderCheckBoxes(headersToBeValidated);
        waitForPageLoadingToComplete();

        final List<Map<String,String>> dataDisplayedOnUI = subscriberEventAnalysis.getAllPagesData();
        subscriberEventAnalysis.uncheckOptionalHeaderCheckBoxes(headersToBeValidated);

        boolean assertFlag = AggregrationHandlerUtil.getAggregrationResult(dataDisplayedOnUI, headersToBeValidated, 
        		GuiStringConstants.IMSI, imsi, GuiStringConstants.CALLFORWARDING);
		assertTrue( DataIntegrityConstants.AGGREGRATION_ERROR_MESSAGE + imsi, assertFlag);
    }
